...
public class MedicionService : IMedicionService
{
        ...
        DateTime hasta)
    {
        return await _context.Mediciones
            ...
            {
                Fecha = grupo.Key,
                PromedioSistolica = grupo.Average(
                    m => m.PresionSistolica),

                PromedioDiastolica = grupo.Average(
                    m => m.PresionDiastolica),
    ...
    public async Task<List<PromedioFranjaDto>>
        ObtenerPromediosPorFranjaAsync(
            DateTime desde,
            DateTime hasta)
    {
        ...
        return mediciones
            .GroupBy(m =>
                ObtenerFranjaHoraria(m.FechaHora.Hour))
            .Select(grupo => new PromedioFranjaDto
            {
                Franja = grupo.Key,
            ...
            .OrderBy(resultado =>
                resultado.Franja == "Mañana"
                    ? 1
                    : resultado.Franja == "Tarde"
                        ? 2
                        : 3)
            .ToList();
    }
